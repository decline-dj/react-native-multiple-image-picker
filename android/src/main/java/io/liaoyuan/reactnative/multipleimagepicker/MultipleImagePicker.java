...
public class MultipleImagePicker extends ReactContextBaseJavaModule {
    ...
    public void launchImageGallery(ReadableMap options, final Promise promise) {
        ...
        if (single) {
            ...
            if (options.hasKey("allowTakePhoto") && !options.getBoolean("allowTakePhoto")) {
                rxGalleryFinal.multiple().hideCamera();
            } else {
                rxGalleryFinal.multiple();
            }
            rxGalleryFinal.maxSize(maxImagesCount).imageLoader(ImageLoaderType.PICASSO).subscribe(new RxBusResultDisposable<ImageMultipleResultEvent>() {
                @Override
